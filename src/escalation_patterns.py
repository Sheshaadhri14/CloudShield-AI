...
ESCALATION_TECHNIQUES = [
    ...
    # T13–T21: Completed from Rhino Security Labs research (2019)
    EscalationTechnique(
        technique_id="T13",
        name="EC2UserData",
        required_permissions=["ec2:RunInstances", "iam:PassRole"],
        description="Run EC2 instance with IAM role via PassRole; inject payload via UserData",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T14",
        ...
        description="Create CloudFormation stack with privileged role to provision admin resources",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T15",
        name="DataPipelinePassRole",
        required_permissions=["datapipeline:CreatePipeline", "datapipeline:PutPipelineDefinition",
                              "datapipeline:ActivatePipeline", "iam:PassRole"],
        description="Create Data Pipeline with admin PassRole to run arbitrary commands",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T16",
        name="GlueDevEndpointPassRole",
        required_permissions=["glue:CreateDevEndpoint", "iam:PassRole"],
        description="Create Glue DevEndpoint with privileged role to access attached permissions",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T17",
        name="SageMakerNotebookPassRole",
        required_permissions=["sagemaker:CreateNotebookInstance", "iam:PassRole"],
        description="Create SageMaker notebook with privileged role for code execution",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T18",
        name="UpdateFunctionConfiguration",
        required_permissions=["lambda:UpdateFunctionConfiguration", "iam:PassRole"],
        description="Change execution role of existing Lambda to one with higher privileges",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T19",
        name="LambdaAddPermission",
        required_permissions=["lambda:AddPermission", "lambda:InvokeFunction"],
        description="Add resource-based policy to Lambda allowing external invocation for lateral movement",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T20",
        name="CodeBuildProjectPassRole",
        required_permissions=["codebuild:CreateProject", "codebuild:StartBuild", "iam:PassRole"],
        description="Create CodeBuild project with privileged role to run arbitrary build commands",
        severity="High"
    ),
    EscalationTechnique(
        technique_id="T21",
        name="STSAssumeRoleChain",
        required_permissions=["sts:AssumeRole"],
        description="Assume a role that itself has escalation permissions, chaining privilege escalation",
        severity="High"
    ),
]
